    ...
    <div className='py-12 px-5'>
      <div>
        ...
        <div className='flex items-center gap-2 flex-wrap'>
          {
            genres.map((item) => {
              return <div onClick={() => setSelectedGenre(item.id)} key={item.id} className={`px-3  cursor-pointer font-medium text-white py-1.5 rounded-md border border-[#1F1F1F] bg-[#0F0F0F] ${item.id === selectedGenre ? 'linear-bg-noHover' : 'linear-bg'}`}>
                {item.name}
              </div>
            })
          }
        ...
        <div className='grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 grid-cols-2 2xl:grid-cols-6 gap-3 mt-5'>
          ...
          {
            loading && Array(19).fill(null).map(() => { return <div className='h-full w-full'><CardSkeleton /></div> })
          }
        </div>
        <div className='w-full text-center'>
          <button onClick={() => updatePageFunc()} className="font-medium linear-bg cursor-pointer text-[15px] md:text-[16px] lg:text-[17px] bg-[#0F0F0F] border border-[#1F1F1F] text-gray-300 mt-5 mx-auto px-3 py-2 rounded-md">
            Load more
          </button>
        </div>
      </div>
