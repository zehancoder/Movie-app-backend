...
import { errorMsgState, likeMoviesState } from "../toolkit/slice";
      ...
      <div className=" relative movieDetail">
        ...
        <div className=" overflow-hidden transition  duration-300 relative">
          ...
          <div className="h-full w-full detailDiv z-10  absolute top-0 left-0 transition duration-300">
            ...
            >
              <Link
                to={`/movies/vedio/${videoId}`}
                className="flex cursor-pointer items-center gap-2 px-2 py-2  transition-all duration-300 border  rounded-full bg-[#fff]"
              >
              ...
              <div className="flex items-center gap-2 text-white text-xl mt-3 justify-evenly">
                <div
                  onClick={() => likeVedio(videoId)}
                  className={`${
                    likeMovies?.some((data) => {
                      if (data.movieId == videoId.toString()) {
                        return true;
                      }
                    })
                      ? "bg-[#0513e0]"
                      : "bg-transparent"
                  } cursor-pointer px-1.5 border border-gray-400 rounded-md py-1`}
                >
                  {likeMovies?.some((data) => {
                    if (data.movieId == videoId.toString()) {
                      return true;
                ...
                <div
                  className={`${save ? "bg-[#3d4afe]" : "bg-transparent"} cursor-pointer px-1.5 border border-gray-400 rounded-md py-1`}
                >
